...
export const useMovCajaStore = create((set,get) => ({
  ...
  totalEfectivoTotalCaja: 0,
   // Actualiza automáticamente el total efectivo total caja
  ...
  setTotalEfectivoCajaSinVentas: (value) => {
    set({ totalEfectivoCajaSinVentas: value });
    get().updateTotalEfectivoTotalCaja(); // Recalcular el total
  },
  setTotalVentasEfectivo: (value) => {
    set({ totalVentasEfectivo: value });
    get().updateTotalEfectivoTotalCaja(); // Recalcular el total
  },
  ...
  mostrarEfectivoSinVentasMovcierrecaja: async (p) => {
    ...
    // Sumar la columna "monto" solo para los movimientos de tipo "ingreso"
